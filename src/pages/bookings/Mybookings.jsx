...
import { getMyBookedPropertiesFrontend } from "../../redux/apis/properties";
...
const fetchBookings = async () => {
    ...
    setError(null);
    try {
      const res = await getMyBookedPropertiesFrontend({
        page,
        limit,
        propertyName,
        bookingPurpose,
...
const handleDownloadReceipt = async (bookingId) => {
  ...
  try {
    const response = await apiClient.get(
      `/properties/bookings/${bookingId}/receipt`,
      {
        responseType: "blob",   // 👈 ye zaroori hai for file
    ...
    window.URL.revokeObjectURL(url); // 👈 ye bilkul sahi hai
                          ...
                          <td className="px-2 py-5 text-center">
                            <div className="space-y-1">
                              ...
                              <div className="text-xs text-gray-500">
                                ₹
                                {Math.round(
                                  booking.baseAmount / booking.bookedForDaysCount
...
<td className="px-6 py-4 text-center">
  ...
  ) : booking.bookingStatus === "1" && booking.daysLeftForCancellation >= 30 ? (
    <button
